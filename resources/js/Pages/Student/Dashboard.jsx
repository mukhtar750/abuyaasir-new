...
export default function StudentDashboard({ enrollments, campaigns, exploreCourses, upcomingCbts, cbtResults, stats, flash, transactions, upcomingClasses = [], bankDetails }) {
                            ...
                            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                                ...
                                >
                                    ...
                                    <div className="bg-[#1A3C5E]/20 p-4 rounded-xl border border-[#F4A623]/20">
                                        <p className="text-sm text-gray-300">To enroll in <span className="text-[#F4A623] font-bold">{selectedCourse.title}</span>, please pay <span className="text-[#2ECC8C] font-bold">&#8358;{parseFloat(selectedCourse.price).toLocaleString()}</span> to the bank details below and upload your receipt.</p>
                                    </div>

                                    {/* Dynamic Bank Details Display */}
                                    <div className="bg-white/5 border border-white/10 rounded-2xl p-5 space-y-3">
                                        <div className="flex justify-between items-center">
                                            <span className="text-[10px] uppercase text-gray-500 font-bold">Bank Name</span>
                                            <span className="text-sm text-white font-medium">{bankDetails?.bank_name || 'N/A'}</span>
                                        </div>
                                        <div className="flex justify-between items-center">
                                            <span className="text-[10px] uppercase text-gray-500 font-bold">Account Name</span>
                                            <span className="text-sm text-white font-medium">{bankDetails?.account_name || 'N/A'}</span>
                                        </div>
                                        <div className="flex justify-between items-center">
                                            <span className="text-[10px] uppercase text-gray-500 font-bold">Account Number</span>
                                            <span className="text-sm text-[#F4A623] font-black tracking-widest">{bankDetails?.account_number || 'N/A'}</span>
                                        </div>
                                    </div>
                                    ...
                                    {bankDetails?.support_whatsapp && (
                                        <div className="pt-4 text-center">
                                            <p className="text-[10px] text-gray-500 uppercase tracking-widest mb-2">Need help with payment?</p>
                                            <a 
                                                href={`https://wa.me/${bankDetails.support_whatsapp}?text=Hello Admin, I need help with payment for ${selectedCourse.title}`}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="inline-flex items-center text-[#2ECC8C] hover:text-[#2ECC8C]/80 transition text-xs font-bold"
                                            >
                                                <svg className="w-4 h-4 mr-1.5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.353-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.87 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.87 0 012.893 6.994c-.003 5.45-4.438 9.884-9.885 9.884m8.413-18.297A11.815 11.87 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.87 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.87 0 00-3.48-8.413Z"/></svg>
